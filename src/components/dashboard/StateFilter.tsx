...
import type { WorkItemState } from '@/types/workItem'
import { cn } from '@/lib/utils'
import { STATE_COLORS } from '@/lib/colors'

const ALL_STATES: WorkItemState[] = ['New', 'Active', 'Resolved', 'Closed', 'Removed']

interface StateFilterProps {
  selected: Set<WorkItemState>
  onChange: (states: Set<WorkItemState>) => void
}

export function StateFilter({ selected, onChange }: StateFilterProps) {
  const [open, setOpen] = useState(false)

  const allSelected = selected.size === ALL_STATES.length
  ...
  function toggleState(state: WorkItemState) {
        ...
        <Command>
          ...
          <CommandGroup heading="States">
            {ALL_STATES.map((state) => {
              const isChecked = selected.has(state)
              return (
                <CommandItem key={state} onSelect={() => toggleState(state)}>
